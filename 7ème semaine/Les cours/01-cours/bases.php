...
//Les objets sont un autre type de données. Un peu à la manière d'un array, un objet permet de regrouper des informations.
//Ici, on parlera de propriétés (=variables) et de méthodes (=functions)

class Etudiant{ //une classe est un constructeur d'objet !

	public $prenom = 'Jeremie';
	//public : permet de dire que la propriété est accessible partout (il existe 'protected' et 'private')
...
//Un objet est conteneur symbolique qui possède sa propre existence et incopore des informations (=propriétés) et des mécanimes (=méthodes)
$etudiant1 = new Etudiant();

//Le mot clé "new" permet d'instancier (=déployer) la classe Etudiant, ce qui va créer une instance (=objet). On se servira de ce qu'il y a à l'intérieur de la classe VIA objet

//echo $etudiant1; //ERROR FATAL, là aussi, comme pour les arrays, ON NE PEUT PAS AFFICHER LES INFOS DE L'OBJET tel quel
...
//Affichage de l'info 'Jeremie' :
echo $etudiant1->prenom . '<br>';

//Dans un array, on va piocher les informations avec des crochets ALORS qu'ici, pour les objets, on utilisera la fleche '->' pour accéder aux informations (=propriétés) de la classe VIA l'OBJET.
...
echo '<pre>';
	...
	//get_class_methods( arg ) : fonction qui permet de voir les méthodes disponibles d'un objet
echo '</pre>';

//Utilisation d'une méthode d'un objet :
echo $etudiant1->pays();

	//pays() est une méthode (=function) donc il faut OBLIGATOIRE mettre les PARENTHSES
...
//Inclusion du fichier nommé 'exemple.inc.php' (qui correspond à l'URl du fichier à inclure qui est ici au meme niveau que le fichier base.php) A CET ENDROIT PRECIS car l'interpréteur php interprète les lignes de codes au fur et à mesure.

echo "Deuxième fois : <br>";
include_once "exemple.inc.php"; //le "once" permet de vérifier si le fichier a déjà été inclue ET si c'est le cas, il ne le ré-inclus pas
...
//Différence entre 'include' et 'require' :
	...
	//Include : fera une erreur, si il a y un problème avec le nom du fichier (ou l'url) à inclure et CONTINUERA l'exécution du script
	//require : fera une erreur, si il a y un problème avec le nom du fichier (ou l'url) à inclure et STOPPERA l'exécution du script
?>
</body>
</html>